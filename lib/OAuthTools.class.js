...
module.exports = function OAuthTools() {
    ...
    this.encodeURIComponent = function(uriComponent) {
        return (
    ...
    this.decodeURIComponent = function(uriComponent) {
        return (
    ...
    this.createHMACSignature = function(hashMethod, signingKey, signatureBase, digestMethod) {
        return (crypto.createHmac(hashMethod, signingKey).update(signatureBase).digest(digestMethod));
    };

    this.createNonce = function(length) {
        var nonce = "",
            i;

        for (i = 0; i < length; ++i) {
            var randomNumber = Math.floor(Math.random() * nonceChars.length);
    ...
    this.joinParams = function(params, seperator, valuesWithQutes) {
