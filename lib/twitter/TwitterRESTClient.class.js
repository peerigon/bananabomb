...
var _ = require("underscore"),
    TwitterSignature = require("./TwitterSignature.class");

/**
 * A twitter REST-API-Client.
 *
 * @param {TwitterUser} twitterUser
 * @param {TwitterConsumer} twitterConsumer
 * @param {TwitterSignature} twitterSignature (optional for tests)
 * @param {oAuthTools} oAuthTools (optional for tests)
 * @param {https} https (optional for tests)
 */
function TwitterRESTClient(twitterUserAccessToken, twitterUserAccessTokenSecret, twitterConsumerKey, twitterConsumerSecret, twitterSignature, oAuthTools, https) {
    ...
    function __construct() {
        if (!twitterUserAccessToken || !twitterUserAccessTokenSecret || !twitterConsumerKey || !twitterConsumerSecret) {
            throw new Error("You must provide an instance of twitterUserAccessToken, twitterUserAccessTokenSecret, twitterConsumerKey, twitterConsumerSecret and TwitterSignature on construction.");
        }
        twitterSignature = twitterSignature || new TwitterSignature();
        oAuthTools = oAuthTools || require("../oAuthTools");
        https = https || require("https");
        signingKey          = twitterSignature.createSigningKey(twitterConsumerSecret, twitterUserAccessTokenSecret);
        APIVersion          = 1;
    ...
    function __getFreshBaseOAuthParams() {
        return {
            ...
            "oauth_token":              twitterUserAccessToken,
            "oauth_version":            "1.0"
        };
    }
